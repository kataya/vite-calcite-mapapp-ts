...
map.when(() => {
    ...
    (document.querySelector("#item-rating") as CalciteRating).value = avgRating;
    ...
    const handleActionBarClick = (event :MouseEvent) => { //({ target }) => {
        ...
        if (activeWidget) {
            (document.querySelector(`[data-action-id=${activeWidget}]`) as CalciteAction).active = false;
            (document.querySelector(`[data-panel-id=${activeWidget}]`) as CalcitePanel).hidden = true;
        }

        const nextWidget = target.dataset.actionId;
        if (nextWidget !== activeWidget) {
            (document.querySelector(`[data-action-id=${nextWidget}]`) as CalciteAction).active = true;
            (document.querySelector(`[data-panel-id=${nextWidget}]`) as CalcitePanel).hidden = false;
